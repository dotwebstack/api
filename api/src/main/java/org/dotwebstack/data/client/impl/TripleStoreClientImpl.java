 ...
 */
public abstract class TripleStoreClientImpl<R extends TripleStoreRepository> implements
    TripleStoreClient {
  ...
  private Model getStatementsAsModel(String subject, String predicate, Object object) {
    return getStatements(subject, predicate, object,
        result -> Iterations.addAll(result, new LinkedHashModel()));
  }

  private <T> T getStatements(String subject, String predicate, Object object,
      Function<RepositoryResult<Statement>, T> consumer) {
    return repository.performQuery(connection -> {
          ...
          try (RepositoryResult<Statement> result = connection
              .getStatements(resourceSubject, iriPredicate, valueObject)) {
  ...
  private <T> Value createLiteral(T object, ValueFactory factory) {
    try {
      Class argument = ClassUtils.isPrimitiveWrapper(object.getClass()) ? ClassUtils
          .wrapperToPrimitive(object.getClass()) : object.getClass();
