...
public class InformationProductControllerTest {
    ...
    public void testFetchesParametersFromRequest() {
        ...
        assertEquals("Parameter should be present", "parameter", value.keySet().stream().findFirst().get());
    ...
    public void testFetchesOnlyTheFirstValue() {
        ...
        assertEquals("Size should be 1", 1, value.size());
        assertEquals("Parameter should be present", "parameter", value.keySet().stream().findFirst().get());
        assertEquals("Value1 should been filled, value2 ignored", "value1", value.values().stream().findFirst().get());
    }
}
