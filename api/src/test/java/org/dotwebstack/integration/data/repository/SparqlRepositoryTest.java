...
public class SparqlRepositoryTest {
    ...
    public void testDbpediaEndpoint() {
        ...
        //assert
        assertEquals("There should have been 1 statements added to the in SQARQL database", 1, result.size());
        assertEquals("It should be the Netherlands", "http://dbpedia.org/resource/Netherlands", result.objects().stream().map(Value::stringValue).toArray()[0]);
    }
